...
public class IssuesController extends Controller {
	...
	static void doCloseIssues() {
		...
		for (String id : ids) {
			Issue issue = RepositoryFactory.getRepository().findIssueById(Long.parseLong(id));
			issue.setProcessingState(IssueProcessingState.CLOSED);
	 ...
	 * 
	 * @param sorting
	 *            die Sortierung fuer die Issues.
	 * @param pagination
	 *            das angeforderte Subset von Issues.
	 * @param filter
	 *            die Menge an Filterkriterien die ein zurueckgeliefertes Issue
	 *            erfuellen muss.
	 * @param stateBinder
	 *            schrenkt die Ergebnismenge aufgrund des Issue states ein.
	 ...
	 * 
	 * @param id
	 *            die Id des angeforderten Issues.
	...
	static void doUnassignIssue() {
		...
		issue.setProcessingState(IssueProcessingState.OPEN);
		RepositoryFactory.getRepository().save(issue);
	}
	...
	private static void updateIssueFromRequestBody(Issue issue, Map<String, String[]> body) {
		...
		issue.setComment(body.get("comment")[0]);
		RepositoryFactory.getRepository().save(issue);
	}
	...
	private final static class IssueProcessingStatePredicate implements Predicate {
		...
		public boolean evaluate(Object arg0) {
			Issue issue = (Issue) arg0;
			return state.equals(issue.getProcessingState());
		}
	}
